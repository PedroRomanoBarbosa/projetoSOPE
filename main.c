...
int main(int argc, char* args[]){
    ...
    if(count>0){
        int acc=0;
        DIR* dir2 = opendir(args[1]);
        struct dirent* dp2;
        while((dp2 = readdir(dir2)) != NULL){
            if( strstr(dp2->d_name,".txt") != NULL)
                if( (strcmp(dp2->d_name,"0_ReadMe.txt") != 0) 
                    && (strcmp(dp2->d_name, "words.txt") != 0) 
                    && (strcmp(dp2->d_name, "index.txt") != 0)){
                    chapters[acc]=dp2->d_name;
                    acc++;
                }
